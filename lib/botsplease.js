    ...
    if (nameArray.indexOf(message.text.toLowerCase()) > -1) {
      ...
      if (voteData[message.user].votes.length < 2) {
         this.postMessage(message.user, "Thanks! You've voted for " + message.text + ". Enter the name of another speaker to lodge your second vote:", {as_user: true});
          ...
          this.postMessage(message.user, "Thanks! You've voted for " + message.text + ". You've reaching the maximum number of votes. Type `tally` to see the current tally!", {as_user: true});
      ...
      this.postMessage(message.user, "The tally is: " + this._checkTally(), {as_user: true});
  } else if (voteData[message.user].voting === false) {
      this.postMessage(message.user, "Hi there, I'm BotsPlease. You can check the list of talks using `vote` or check the current tally with `tally`.", {as_user: true});
...
BotsPlease.prototype._voteSuccess = function() {
  this.postMessageToChannel(this.channels[0].name, 'Yayyyy someone voted!!!', {as_user: false});
};
...
// BotsPlease.prototype._showOptions = function(message) {
//   voteData[message.user].voting = true;
// };
